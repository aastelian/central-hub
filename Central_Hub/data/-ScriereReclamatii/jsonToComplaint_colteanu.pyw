...
import time
import tkinter
from MainMenu_colteanu import Main
import os
...
def generateComplaintDoc(textToPv,complaint_number,date,supplier):
    ...
    sheet1.insert_bitmap(os.path.join(rootPath, "-ScriereReclamatii/logo_solo.bmp"), 0,7)
    sheet1.insert_bitmap(os.path.join(rootPath, "-ScriereReclamatii/signature_solo.bmp"), 50,6)
    ...
    if os.path.exists(os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/resources")) == False:
        os.makedirs(os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/resources"))
    
    wb.save(os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/resources/pv - {supplier} - {complaint_number} - {date}.xls"))
    
    excel = client.Dispatch("Excel.Application")
    sheets = excel.Workbooks.Open(os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/resources/pv - {supplier} - {complaint_number} - {date}.xls"))
    work_sheets = sheets.Worksheets[0]
    
    supplier_copy = supplier.replace(" ","_")
    
    work_sheets.ExportAsFixedFormat(0, os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/pv-{supplier_copy}-{complaint_number}-{date}.pdf"))
...
def generateExcel(complaints,ans,date):
    ...
    for product in product_list:
        ...
        for n in complaints:
            ...
            if complaints[n]["supplier"] == ans and complaints[n]["product"] == product:
                ...
                sheet1.write(k,0,str(n),central_style)
    ...
    sheet1_pv.insert_bitmap(os.path.join(rootPath,"-ScriereReclamatii/logo.bmp"), 0,4)
    sheet1_pv.insert_bitmap(os.path.join(rootPath,"-ScriereReclamatii/signature.bmp"), 40,4)
    ...
    for product in product_list:
        ...
        for n in complaints:
            ...
            if complaints[n]["supplier"] == ans and complaints[n]["product"] == product:
                ...
                sheet1_pv.write(k,p,n,central_style)
    ...
    wb.save(os.path.join(rootPath,f"-ScriereReclamatii/MailReclamatii/{ans} - {date}.xls"))
    wb_pv.save(os.path.join(rootPath,f"-ScriereReclamatii/MailReclamatii/resources/pv - {ans} - {date}.xls"))
    
    excel = client.Dispatch("Excel.Application")
    sheets = excel.Workbooks.Open(os.path.join(rootPath,f"-ScriereReclamatii/MailReclamatii/resources/pv - {ans} - {date}.xls"))
    work_sheet = sheets.Worksheets[0]
    
    ans_copy = ans.replace(" ","_")
    
    work_sheet.ExportAsFixedFormat(0,os.path.join(rootPath,f"-ScriereReclamatii/MailReclamatii/pv-{ans_copy}-{date}.pdf"))
    
    if os.path.exists(os.path.join(rootPath,f"-_RECLAMATII/{ans_copy}-{date}")) == False:
        os.makedirs(os.path.join(rootPath,f"-_RECLAMATII/{ans_copy}-{date}"))
    work_sheet.ExportAsFixedFormat(0,os.path.join(rootPath,f"-_RECLAMATII/{ans_copy}-{date}/pv-{ans_copy}-{date}.pdf"))
...
class App:
    ...
    def createWindow2(self,list_1):
        ...
        self.topWindow2.geometry("300x600")
...
Cosmin Olteanu
Farmacist - Persoana Responsabila
 
Farmexim S.A.
Str. Malul Rosu nr. 4
077015, Balotesti, Jud. Ilfov
Telefon 0213.086.962
Interior 5762
cosmin.olteanu@farmexim.ro
www.farmexim.ro\n
""".format(dictionary[n][0],dictionary[n][0],dictionary[n][1],dictionary[n][2],dictionary[n][3])
                ...
                eml["Cc"] = "andrei.astelian@farmexim.ro;simona.dumitriu@farmexim.ro;nuti.mladin@farmexim.ro"
...
Interior 5762
cosmin.olteanu@farmexim.ro
www.farmexim.ro\n""".format(dictionary[n][2],dictionary[n][0],dictionary[n][0],dictionary[n][1],dictionary[n][2],dictionary[n][3])
...
Motiv: {dictionary[n][3]}\n"""
            ...
            pathToSave = os.path.join(rootPath, "-ScriereReclamatii/MailReclamatii/")
        ...
        if ans != "":
            ...
            file = open(os.path.join(rootPath, "-ScriereReclamatii/complaints.txt"),"w")
            ...
            file.close()
            file = open(os.path.join(rootPath, "-ScriereReclamatii/complaints.txt"),"r")
            text = file.readlines()
            text_copy = ""
            for n in text:
                line = n.split()
...
Au fost reclamate {quantity_sum} produse {ans} neconforme calitativ.
\n""")
...
www.farmexim.ro\n""")
            ...
            eml["Cc"] = "andrei.astelian@farmexim.ro;simona.dumitriu@farmexim.ro;nuti.mladin@farmexim.ro"
            file.close()
            file = open(os.path.join(rootPath, f"-ScriereReclamatii/MailReclamatii/{ans}.eml"),"w")
            gen = email.generator.Generator(file)
            gen.flatten(eml)
            file.close()
            for n in dictionary:
